...
serve(async (req) => {
  ...
  try {
    ...
    // Get request data - ADD planName to destructuring
    const { planId, mode = "subscription", couponCode = null, planName = null } = await req.json();
    ...
    // Use planName from request or fallback to database plan name
    ...
    const priceData = {
      currency: "gbp",
      product_data: {
        name: finalPlanName // Use the plan name here too
      },
    ...
    // Determine billing cycle for metadata
    const billingCycle = mode === "annual" ? "annual" : "monthly";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/pricing?success=true&plan_name=${encodeURIComponent(finalPlanName)}`, // ADD plan name to success URL
