...
const BlogPostPage = () => {
    ...
    const fetchRelatedPosts = async () => {
      try {
        ...
        const related = allPosts
          ...
          .filter(p => p.discipline === post.discipline || p.category === post.category)
  ...
  const renderContent = (content: string) => {
    if (!content) return <p className="text-gray-500 italic">{t["no-content-available"]}</p>;
      ...
      <main className="container mx-auto px-6 pt-32 pb-16">
        {loading ? (
          ...
          <div className="text-center py-16">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">{t["post-not-found"]}</h1>
          ...
          <>
            ...
            <article className="max-w-4xl mx-auto mb-16">
              {renderContent(getLocalizedContent('content', post.content || ''))}
            </article>
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {relatedPosts.map(relatedPost => (
                    <BlogPostCard key={relatedPost.id} post={relatedPost} hideAuthor />
                  ))}
                </div>
              </div>
            )}
