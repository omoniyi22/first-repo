
// ====== COURSE 2: JERSEY FRESH CCI 3* ======
// export const PARIS_GRAND_INDOOR = {
//     id: "paris_grand_indoor_160",
//     name: "Paris Grand Indoor 1.60m",
//     competition_level: "grand_prix",
//     jumping_discipline: 'showjumping',
//     arena: { inline-size: 100, length: 60 },
//     jumps: [
//         {
//             id: "jump_1",
//             number: 1,
//             x: 45,
//             y: 25,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_2",
//             number: 2,
//             x: 39,
//             y: 12,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.0471975511965976,
//             manualRotation: true
//         },
//         {
//             id: "jump_3",
//             number: 3,
//             x: 17,
//             y: 20,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.308996938995747
//         },
//         {
//             id: "jump_4a",
//             number: "4a",
//             x: 14,
//             y: 5,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_4b",
//             number: "4b",
//             x: 22,
//             y: 5,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_4c",
//             number: "4c",
//             x: 34,
//             y: 5,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_5",
//             number: 5,
//             x: 47,
//             y: 5,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_6",
//             number: 6,
//             x: 45,
//             y: 31,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.8325957145940457
//         },
//         {
//             id: "jump_7",
//             number: 7,
//             x: 29,
//             y: 21,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 2.0943951023931953
//         },
//         {
//             id: "jump_8",
//             number: 8,
//             x: 12,
//             y: 14,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.308996938995747
//         },
//         {
//             id: "jump_9",
//             number: 9,
//             x: 23,
//             y: 30,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.308996938995747
//         },
//         {
//             id: "jump_10a",
//             number: "10a",
//             x: 37,
//             y: 22,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_10b",
//             number: "10b",
//             x: 44,
//             y: 16,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_11a",
//             number: "11a",
//             x: 41,
//             y: 35,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_11b",
//             number: "11b",
//             x: 30,
//             y: 35,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_12",
//             number: 12,
//             x: 8,
//             y: 26,
//             block-size: 0.8,
//             type: "vertical",
//             rotation: -0.2617993877991494
//         }
//     ],
//     metadata: {
//         source: "Paris Grand Indoor Salon du Cheval 2018",
//         complexity: "high",
//         pattern: "flowing",
//         designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine"
//     }
// }

// ====== VARIATION 1: 20° ROTATION ======
// export const PARIS_VARIATION_1_ROTATE_20 = {
//     id: "paris_grand_indoor_var_1_rotate_20",
//     name: "Paris Grand Indoor 1.60m - Rotated 20°",
//     competition_level: "grand_prix",
//     jumping_discipline: 'showjumping',
//     arena: { inline-size: 100, length: 60 },
//     jumps: [
//         {
//             id: "jump_1",
//             number: 1,
//             x: 40.8,
//             y: 28.5,
//             block-size: 1.6,
//             type: "vertical",
//             rotation: -1.2217
//         },
//         {
//             id: "jump_2",
//             number: 2,
//             x: 38.1,
//             y: 16.2,
//             block-size: 1.6,
//             type: "vertical",
//             rotation: 1.3963
//         },
//         {
//             id: "jump_3",
//             number: 3,
//             x: 13.4,
//             y: 22.8,
//             block-size: 1.6,
//             type: "vertical",
//             rotation: 1.9198993877991493,
//             manualRotation: true
//         },
//         {
//             id: "jump_4A",
//             number: "4A",
//             x: 12,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.7452987755982987,
//             manualRotation: true
//         },
//         {
//             id: "jump_4B",
//             number: "4B",
//             x: 22,
//             y: 6,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.2217
//         },
//         {
//             id: "jump_4C",
//             number: "4C",
//             x: 32,
//             y: 8,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.2217
//         },
//         {
//             id: "jump_5",
//             number: 5,
//             x: 46,
//             y: 11,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.2217
//         },
//         {
//             id: "jump_6",
//             number: 6,
//             x: 37.8,
//             y: 34.2,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.9199006122008508,
//             manualRotation: true
//         },
//         {
//             id: "jump_7",
//             number: 7,
//             x: 25,
//             y: 24,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.9199012244017009,
//             manualRotation: true
//         },
//         {
//             id: "jump_8",
//             number: 8,
//             x: 12,
//             y: 16,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.3963006122008506,
//             manualRotation: true
//         },
//         {
//             id: "jump_9",
//             number: 9,
//             x: 19,
//             y: 28,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.6581
//         },
//         {
//             id: "jump_10A",
//             number: "10A",
//             x: 33,
//             y: 25,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.1345012244017012,
//             manualRotation: true
//         },
//         {
//             id: "jump_10B",
//             number: "10B",
//             x: 42,
//             y: 21,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.3963006122008506,
//             manualRotation: true
//         },
//         {
//             id: "jump_11A",
//             number: "11A",
//             x: 34.2,
//             y: 37.8,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.9199
//         },
//         {
//             id: "jump_11B",
//             number: "11B",
//             x: 25,
//             y: 35,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.2217,
//             manualRotation: true
//         },
//         {
//             id: "jump_12",
//             number: 12,
//             x: 10,
//             y: 32,
//             height: 1.6,
//             type: "vertical",
//             rotation: -0.9598975511965975,
//             manualRotation: true
//         }
//     ],
//     combinations: [
//         {
//             id: "4",
//             jumps: ["4A", "4B", "4C"],
//             type: "triple"
//         },
//         {
//             id: "10",
//             jumps: ["10A", "10B"],
//             type: "double"
//         },
//         {
//             id: "11",
//             jumps: ["11A", "11B"],
//             type: "double"
//         }
//     ],
//     metadata: {
//         source: "Paris Grand Indoor Salon du Cheval 2018",
//         complexity: "high",
//         pattern: "flowing",
//         designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine"
//     }
// };

// ====== VARIATION 2: HORIZONTAL MIRROR ======
// export const PARIS_VARIATION_2_MIRROR_H = {
//     id: "paris_grand_indoor_var_2_mirror_h",
//     name: "Paris Grand Indoor 1.60m - Mirrored Horizontal",
//     competition_level: "grand_prix",
//     jumping_discipline: 'showjumping',
//     arena: { width: 100, length: 60 },
//     jumps: [
//         {
//             id: "jump_1",
//             number: 1,
//             x: 15,
//             y: 25,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_2",
//             number: 2,
//             x: 20,
//             y: 14,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_3",
//             number: 3,
//             x: 43,
//             y: 20,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_4A",
//             number: "4A",
//             x: 46,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_4B",
//             number: "4B",
//             x: 38,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_4C",
//             number: "4C",
//             x: 25,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_5",
//             number: 5,
//             x: 13,
//             y: 8,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.0471975511965976,
//             manualRotation: true
//         },
//         {
//             id: "jump_6",
//             number: 6,
//             x: 15,
//             y: 31,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.8325957145940457
//         },
//         {
//             id: "jump_7",
//             number: 7,
//             x: 31,
//             y: 21,
//             height: 1.6,
//             type: "vertical",
//             rotation: -2.0943951023931953
//         },
//         {
//             id: "jump_8",
//             number: 8,
//             x: 48,
//             y: 14,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_9",
//             number: 9,
//             x: 37,
//             y: 30,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_10A",
//             number: "10A",
//             x: 24,
//             y: 21,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_10B",
//             number: "10B",
//             x: 14,
//             y: 17,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.8325957145940457,
//             manualRotation: true
//         },
//         {
//             id: "jump_11A",
//             number: "11A",
//             x: 22,
//             y: 35,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.5707963267948963
//         },
//         {
//             id: "jump_11B",
//             number: "11B",
//             x: 33,
//             y: 35,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_12",
//             number: 12,
//             x: 48,
//             y: 27,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.0471975511965976,
//             manualRotation: true
//         }
//     ],
//     combinations: [
//         {
//             id: "4",
//             jumps: ["4A", "4B", "4C"],
//             type: "triple"
//         },
//         {
//             id: "10",
//             jumps: ["10A", "10B"],
//             type: "double"
//         },
//         {
//             id: "11",
//             jumps: ["11A", "11B"],
//             type: "double"
//         }
//     ],
//     metadata: {
//         source: "Paris Grand Indoor Salon du Cheval 2018 - Variation",
//         complexity: "high",
//         pattern: "flowing mirrored",
//         designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine",
//         variation: "Horizontal mirror flip"
//     }
// };

// ====== VARIATION 3: VERTICAL MIRROR + POSITION ADJUSTMENT ======
// export const PARIS_VARIATION_3_MIRROR_V_POSITION = {
//     id: "paris_grand_indoor_var_3_mirror_v_position",
//     name: "Paris Grand Indoor 1.60m - Vertical Mirror + Position Tweaks",
//     competition_level: "grand_prix",
//     jumping_discipline: 'showjumping',
//     arena: { width: 100, length: 60 },
//     jumps: [
//         {
//             id: "jump_1",
//             number: 1,
//             x: 46.2,
//             y: 15,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_2",
//             number: 2,
//             x: 40,
//             y: 27,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_3",
//             number: 3,
//             x: 18.3,
//             y: 20,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_4A",
//             number: "4A",
//             x: 15.2,
//             y: 35,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_4B",
//             number: "4B",
//             x: 25,
//             y: 35,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_4C",
//             number: "4C",
//             x: 38,
//             y: 34,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_5",
//             number: 5,
//             x: 47,
//             y: 33,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.0471975511965976,
//             manualRotation: true
//         },
//         {
//             id: "jump_6",
//             number: 6,
//             x: 45,
//             y: 10,
//             height: 1.6,
//             type: "vertical",
//             rotation: -2.0943951023931953,
//             manualRotation: true
//         },
//         {
//             id: "jump_7",
//             number: 7,
//             x: 30.8,
//             y: 19,
//             height: 1.6,
//             type: "vertical",
//             rotation: -2.0943951023931953
//         },
//         {
//             id: "jump_8",
//             number: 8,
//             x: 13.2,
//             y: 26,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747
//         },
//         {
//             id: "jump_9",
//             number: 9,
//             x: 19,
//             y: 13,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_10A",
//             number: "10A",
//             x: 31,
//             y: 13,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.0471975511965976,
//             manualRotation: true
//         },
//         {
//             id: "jump_10B",
//             number: "10B",
//             x: 41,
//             y: 20,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.3089969389957472,
//             manualRotation: true
//         },
//         {
//             id: "jump_11A",
//             number: "11A",
//             x: 40,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: -1.308996938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_11B",
//             number: "11B",
//             x: 30,
//             y: 5,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.5707963267948963
//         },
//         {
//             id: "jump_12",
//             number: 12,
//             x: 11,
//             y: 8,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.0471975511965976,
//             manualRotation: true
//         }
//     ],
//     combinations: [
//         {
//             id: "4",
//             jumps: ["4A", "4B", "4C"],
//             type: "triple"
//         },
//         {
//             id: "10",
//             jumps: ["10A", "10B"],
//             type: "double"
//         },
//         {
//             id: "11",
//             jumps: ["11A", "11B"],
//             type: "double"
//         }
//     ],
//     metadata: {
//         source: "Paris Grand Indoor Salon du Cheval 2018 - Variation",
//         complexity: "high",
//         pattern: "flowing vertically mirrored with position adjustments",
//         designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine",
//         variation: "Vertical mirror + 1-2m position adjustments"
//     }
// };

// ====== VARIATION 4: 30° ROTATION + MODERATE HEIGHT CHANGES ======
// export const PARIS_VARIATION_4_ROTATE_30_HEIGHT = {
//     id: "paris_grand_indoor_var_4_rotate_30_height",
//     name: "Paris Grand Indoor 1.60m - Rotated 30° + Height Changes",
//     competition_level: "grand_prix",
//     jumping_discipline: 'showjumping',
//     arena: { width: 100, length: 60 },
//     jumps: [
//         {
//             id: "jump_1",
//             number: 1,
//             x: 55,
//             y: 25,
//             height: 1.5,
//             type: "vertical",
//             rotation: -3.665193877991495
//         },
//         {
//             id: "jump_2",
//             number: 2,
//             x: 43,
//             y: 12,
//             height: 1.65,
//             type: "vertical",
//             rotation: 1.3090006122008508
//         },
//         {
//             id: "jump_3",
//             number: 3,
//             x: 18,
//             y: 26,
//             height: 1.6,
//             type: "vertical",
//             rotation: 1.3090012244017013,
//             manualRotation: true
//         },
//         {
//             id: "jump_4A",
//             number: "4A",
//             x: 12,
//             y: 10,
//             height: 1.55,
//             type: "vertical",
//             rotation: -2.356196938995747,
//             manualRotation: true
//         },
//         {
//             id: "jump_4B",
//             number: "4B",
//             x: 24,
//             y: 6,
//             height: 1.55,
//             type: "vertical",
//             rotation: -1.832598163397448
//         },
//         {
//             id: "jump_4C",
//             number: "4C",
//             x: 35,
//             y: 10,
//             height: 1.55,
//             type: "vertical",
//             rotation: -1.0472
//         },
//         {
//             id: "jump_5",
//             number: 5,
//             x: 41,
//             y: 18,
//             height: 1.7,
//             type: "vertical",
//             rotation: -0.7854006122008506
//         },
//         {
//             id: "jump_6",
//             number: 6,
//             x: 40,
//             y: 33,
//             height: 1.6,
//             type: "vertical",
//             rotation: 2.0944006122008503,
//             manualRotation: true
//         },
//         {
//             id: "jump_7",
//             number: 7,
//             x: 24,
//             y: 21,
//             height: 1.55,
//             type: "vertical",
//             rotation: 2.618,
//             manualRotation: true
//         },
//         {
//             id: "jump_8",
//             number: 8,
//             x: 19,
//             y: 10,
//             height: 1.65,
//             type: "vertical",
//             rotation: -0.5235944901923444,
//             manualRotation: true
//         },
//         {
//             id: "jump_9",
//             number: 9,
//             x: 10,
//             y: 21,
//             height: 1.5,
//             type: "vertical",
//             rotation: 3.1415969389957477
//         },
//         {
//             id: "jump_10A",
//             number: "10A",
//             x: 20,
//             y: 33,
//             height: 1.55,
//             type: "vertical",
//             rotation: 1.5708006122008507,
//             manualRotation: true
//         },
//         {
//             id: "jump_10B",
//             number: "10B",
//             x: 29,
//             y: 30,
//             height: 1.55,
//             type: "vertical",
//             rotation: 1.3090012244017013,
//             manualRotation: true
//         },
//         {
//             id: "jump_11A",
//             number: "11A",
//             x: 41,
//             y: 24,
//             height: 1.7,
//             type: "vertical",
//             rotation: 1.0472024488034024,
//             manualRotation: true
//         },
//         {
//             id: "jump_11B",
//             number: "11B",
//             x: 47,
//             y: 19,
//             height: 1.7,
//             type: "vertical",
//             rotation: -2.356196938995747
//         },
//         {
//             id: "jump_12",
//             number: 12,
//             x: 53,
//             y: 10,
//             height: 1.6,
//             type: "vertical",
//             rotation: 0.5235993877991494
//         }
//     ],
//     combinations: [
//         {
//             id: "4",
//             jumps: ["4A", "4B", "4C"],
//             type: "triple"
//         },
//         {
//             id: "10",
//             jumps: ["10A", "10B"],
//             type: "double"
//         },
//         {
//             id: "11",
//             jumps: ["11A", "11B"],
//             type: "double"
//         }
//     ],
//     metadata: {
//         source: "Paris Grand Indoor Salon du Cheval 2018 - Variation",
//         complexity: "high",
//         pattern: "flowing rotated 30° with height variation",
//         designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine",
//         variation: "30° clockwise rotation + height range 1.50m-1.70m"
//     }
// };






export const COURSE_DATA = [

    // Beginner Showjumping Courses Courses
    {
        id: "intro_showjumping_course",
        name: "Intro Showjumping Course",
        competition_level: "intro",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: 1752050273000,
                x: 20,
                y: 19,
                type: "vertical",
                number: 1,
                height: 0.5,
                rotation: 0
            },
            {
                id: 1752050280580,
                x: 28,
                y: 22,
                type: "vertical",
                number: 2,
                height: 0.5,
                rotation: 0.5235987755982988,
                manualRotation: true
            },
            {
                id: 1752050295869,
                x: 52,
                y: 27,
                type: "vertical",
                number: 3,
                height: 0.5,
                rotation: 0
            },
            {
                id: 1752050298730,
                x: 52,
                y: 11,
                type: "vertical",
                number: 4,
                height: 0.5,
                rotation: 0
            },
            {
                id: 1752050304985,
                x: 7,
                y: 14,
                type: "vertical",
                number: '5a',
                height: 0.5,
                rotation: 0
            },
            {
                id: 1752050306776,
                x: 7,
                y: 24,
                type: "vertical",
                number: '5b',
                height: 0.5,
                rotation: 0
            },
            {
                id: 1752050449076,
                x: 31,
                y: 32,
                type: "vertical",
                number: 6,
                height: 0.5,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752050510368,
                x: 34,
                y: 26,
                type: "vertical",
                number: 7,
                height: 0.5,
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752050530363,
                x: 28,
                y: 15,
                type: "vertical",
                number: 8,
                height: 0.5,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752050542707,
                x: 33,
                y: 8,
                type: "vertical",
                number: 9,
                height: 0.5,
                rotation: -1.5707963267948963,
                manualRotation: true
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "manual",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 9
        }
    },
    {
        id: "intro_showjumping_var_1_mirror_v_position",
        name: "Intro Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "intro",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 21.2,
                y: 21,
                height: 0.8,
                type: "vertical",
                rotation: 0
            },
            {
                id: "jump_2",
                number: 2,
                x: 29.1,
                y: 18,
                height: 0.8,
                type: "vertical",
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 52,
                y: 13,
                height: 0.8,
                type: "vertical",
                rotation: 0
            },
            {
                id: "jump_4",
                number: 4,
                x: 51.8,
                y: 29,
                height: 0.8,
                type: "vertical",
                rotation: 0
            },
            {
                id: "jump_5A",
                number: "5A",
                x: 7,
                y: 26,
                height: 0.8,
                type: "vertical",
                rotation: 0
            },
            {
                id: "jump_5B",
                number: "5B",
                x: 7,
                y: 18,
                height: 0.8,
                type: "vertical",
                rotation: 0
            },
            {
                id: "jump_6",
                number: 6,
                x: 30.8,
                y: 8,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_7",
                number: 7,
                x: 35,
                y: 14,
                height: 0.8,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_8",
                number: 8,
                x: 29,
                y: 25,
                height: 0.8,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_9",
                number: 9,
                x: 34,
                y: 33,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            }
        ],
        combinations: [
            {
                id: "5",
                jumps: ["5A", "5B"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 9,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },

    // Basic Showjumping Courses
    {
        id: "basic_showjumping_course",
        name: "Basic Showjumping Course",
        competition_level: "novice",
        jumping_discipline: 'showjumping',
        arena: { width: 70, length: 45 },
        jumps: [
            {
                id: 1752052723002,
                x: 18,
                y: 40,
                type: "vertical",
                number: 1,
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052730333,
                x: 41,
                y: 40,
                type: "vertical",
                number: 2,
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052738715,
                x: 47,
                y: 16,
                type: "vertical",
                number: 3,
                height: 0.8,
                rotation: 0.7853981633974481,
                manualRotation: true
            },
            {
                id: 1752052748885,
                x: 27,
                y: 30,
                type: "vertical",
                number: '4a',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052751724,
                x: 18,
                y: 30,
                type: "vertical",
                number: '4b',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052763942,
                x: 18,
                y: 14,
                type: "vertical",
                number: 5,
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052769635,
                x: 42,
                y: 30,
                type: "vertical",
                number: 6,
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052782126,
                x: 49,
                y: 9,
                type: "vertical",
                number: '7a',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052785773,
                x: 41,
                y: 9,
                type: "vertical",
                number: '7b',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052812784,
                x: 20,
                y: 22,
                type: "vertical",
                number: 8,
                height: 0.8,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052826250,
                x: 54,
                y: 5,
                type: "vertical",
                number: 9,
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052834503,
                x: 50,
                y: 25,
                type: "vertical",
                number: 10,
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "4",
                jumps: ["4a", "4b"],
                type: "double"
            },
            {
                id: "7",
                jumps: ["7a", "7b"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 9,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "basic_showjumping_var_2_mirror_h",
        name: "Basic Showjumping Course - Mirrored Horizontal",
        competition_level: "novice",
        jumping_discipline: 'showjumping',
        arena: { width: 70, length: 45 },
        jumps: [
            {
                id: 1752052723004,
                x: 52,
                y: 40,
                type: "vertical",
                number: 1,
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052730335,
                x: 29,
                y: 40,
                type: "vertical",
                number: 2,
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052738717,
                x: 23,
                y: 16,
                type: "vertical",
                number: 3,
                height: 1.0,
                rotation: -0.7853981633974481,
                manualRotation: true
            },
            // COMBINATION 4A-4B (Mirrored together as a unit)
            {
                id: 1752052748887,
                x: 43,
                y: 30,
                type: "vertical",
                number: '4a',
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "4"
            },
            {
                id: 1752052751726,
                x: 52,
                y: 30,
                type: "vertical",
                number: '4b',
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "4"
            },
            {
                id: 1752052763944,
                x: 52,
                y: 14,
                type: "vertical",
                number: 5,
                height: 1.0,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052769637,
                x: 28,
                y: 30,
                type: "vertical",
                number: 6,
                height: 1.0,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            // COMBINATION 7A-7B (Mirrored together as a unit)
            {
                id: 1752052782128,
                x: 21,
                y: 9,
                type: "vertical",
                number: '7a',
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "7"
            },
            {
                id: 1752052785775,
                x: 29,
                y: 9,
                type: "vertical",
                number: '7b',
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "7"
            },
            {
                id: 1752052812786,
                x: 50,
                y: 22,
                type: "vertical",
                number: 8,
                height: 1.0,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052826252,
                x: 16,
                y: 5,
                type: "vertical",
                number: 9,
                height: 1.0,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052834505,
                x: 20,
                y: 25,
                type: "vertical",
                number: 10,
                height: 1.0,
                rotation: 0.7853981633974483,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "4",
                jumps: ["4a", "4b"],
                type: "double"
            },
            {
                id: "7",
                jumps: ["7a", "7b"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 10,
            variation: "Horizontal mirror flip"
        }
    },
    {
        id: "basic_showjumping_var_4_mirror_v_position",
        name: "Basic Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "novice",
        jumping_discipline: 'showjumping',
        arena: { width: 70, length: 45 },
        jumps: [
            {
                id: 1752052723006,
                x: 19.2,
                y: 5,
                type: "vertical",
                number: 1,
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052730337,
                x: 42.1,
                y: 5,
                type: "vertical",
                number: 2,
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052738719,
                x: 48.2,
                y: 29,
                type: "vertical",
                number: 3,
                height: 1.0,
                rotation: 0.7853981633974481,
                manualRotation: true
            },
            // COMBINATION 4A-4B (Vertically mirrored and adjusted together)
            {
                id: 1752052748889,
                x: 28.1,
                y: 15,
                type: "vertical",
                number: '4a',
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true,
                combination: "4"
            },
            {
                id: 1752052751728,
                x: 19.2,
                y: 15,
                type: "vertical",
                number: '4b',
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true,
                combination: "4"
            },
            {
                id: 1752052763946,
                x: 19.1,
                y: 31,
                type: "vertical",
                number: 5,
                height: 1.0,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052769639,
                x: 43.2,
                y: 15,
                type: "vertical",
                number: 6,
                height: 1.0,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            // COMBINATION 7A-7B (Vertically mirrored and adjusted together)
            {
                id: 1752052782130,
                x: 50.1,
                y: 36,
                type: "vertical",
                number: '7a',
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true,
                combination: "7"
            },
            {
                id: 1752052785777,
                x: 42.2,
                y: 36,
                type: "vertical",
                number: '7b',
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true,
                combination: "7"
            },
            {
                id: 1752052812788,
                x: 21.1,
                y: 23,
                type: "vertical",
                number: 8,
                height: 1.0,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752052826254,
                x: 55.2,
                y: 40,
                type: "vertical",
                number: 9,
                height: 1.0,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752052834507,
                x: 51.1,
                y: 20,
                type: "vertical",
                number: 10,
                height: 1.0,
                rotation: -0.7853981633974483,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "4",
                jumps: ["4a", "4b"],
                type: "double"
            },
            {
                id: "7",
                jumps: ["7a", "7b"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 10,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },

    // Intermediate Showjumping Courses
    {
        id: "intermediate_showjumping_course",
        name: "Intermediate Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "elementary",
        jumping_discipline: 'showjumping',
        arena: { width: 80, length: 50 },
        jumps: [
            {
                id: 1752055053137,
                x: 35,
                y: 45,
                type: "vertical",
                number: 1,
                height: 0.9,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752055067882,
                x: 66,
                y: 40,
                type: "vertical",
                number: 2,
                height: 0.9,
                rotation: 0.5235987755982989,
                manualRotation: true
            },
            {
                id: 1752055080069,
                x: 53,
                y: 14,
                type: "vertical",
                number: '3a',
                height: 0.9,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055087331,
                x: 46,
                y: 20,
                type: "vertical",
                number: '3b',
                height: 0.9,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055097334,
                x: 30,
                y: 33,
                type: "vertical",
                number: 4,
                height: 0.9,
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055123514,
                x: 21,
                y: 20,
                type: "vertical",
                number: 5,
                height: 0.9,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055140458,
                x: 53,
                y: 40,
                type: "vertical",
                number: 6,
                height: 0.9,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055147302,
                x: 71,
                y: 18,
                type: "vertical",
                number: 7,
                height: 0.9,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055151965,
                x: 44,
                y: 5,
                type: "vertical",
                number: '8a',
                height: 0.9,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752055155827,
                x: 34,
                y: 5,
                type: "vertical",
                number: '8b',
                height: 0.9,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752055159864,
                x: 25,
                y: 5,
                type: "vertical",
                number: '8c',
                height: 0.9,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752055179807,
                x: 38,
                y: 37,
                type: "vertical",
                number: 9,
                height: 0.9,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055266024,
                x: 49,
                y: 31,
                type: "vertical",
                number: 10,
                height: 0.9,
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055183086,
                x: 64,
                y: 30,
                type: "vertical",
                number: 11,
                height: 0.9,
                rotation: 1.5707963267948963,
                manualRotation: true
            },

        ],
        combinations: [
            {
                id: "3",
                jumps: ["3a", "3b"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8a", "8b", "8c"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 10,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "intermediate_showjumping_var_1_mirror_h",
        name: "Intermediate Showjumping Course - Mirrored Horizontal",
        competition_level: "elementary",
        jumping_discipline: 'showjumping',
        arena: { width: 80, length: 50 },
        jumps: [
            {
                id: 1752055053139,
                x: 45,
                y: 45,
                type: "vertical",
                number: 1,
                height: 1.2,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752055067884,
                x: 14,
                y: 40,
                type: "vertical",
                number: 2,
                height: 1.2,
                rotation: -0.5235987755982989,
                manualRotation: true
            },
            // COMBINATION 3A-3B (Mirrored together as a unit)
            {
                id: 1752055080071,
                x: 27,
                y: 14,
                type: "vertical",
                number: '3A',
                height: 1.2,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "3"
            },
            {
                id: 1752055087333,
                x: 34,
                y: 20,
                type: "vertical",
                number: '3B',
                height: 1.2,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "3"
            },
            {
                id: 1752055097336,
                x: 50,
                y: 33,
                type: "vertical",
                number: 4,
                height: 1.2,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055123516,
                x: 59,
                y: 20,
                type: "vertical",
                number: 5,
                height: 1.2,
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055140460,
                x: 27,
                y: 40,
                type: "vertical",
                number: 6,
                height: 1.2,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055147304,
                x: 9,
                y: 18,
                type: "vertical",
                number: 7,
                height: 1.2,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            // COMBINATION 8A-8B-8C (Triple mirrored together as a unit)
            {
                id: 1752055151967,
                x: 36,
                y: 5,
                type: "vertical",
                number: '8A',
                height: 1.2,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "8"
            },
            {
                id: 1752055155829,
                x: 46,
                y: 5,
                type: "vertical",
                number: '8B',
                height: 1.2,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "8"
            },
            {
                id: 1752055159866,
                x: 55,
                y: 5,
                type: "vertical",
                number: '8C',
                height: 1.2,
                rotation: 1.5707963267948963,
                manualRotation: true,
                combination: "8"
            },
            {
                id: 1752055179809,
                x: 42,
                y: 37,
                type: "vertical",
                number: 9,
                height: 1.2,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752055266026,
                x: 31,
                y: 31,
                type: "vertical",
                number: 10,
                height: 1.2,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752055183088,
                x: 16,
                y: 30,
                type: "vertical",
                number: 11,
                height: 1.2,
                rotation: -1.5707963267948963,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3A", "3B"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8A", "8B", "8C"],
                type: "triple"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
            variation: "Horizontal mirror flip"
        }
    },
    {
        id: "intermediate_showjumping_var_3_mirror_v_position",
        name: "Intermediate Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "elementary",
        jumping_discipline: 'showjumping',
        arena: { width: 80, length: 50 },
        jumps: [
            {
                id: "1",
                number: 1,
                x: 36.2,
                y: 5,
                height: 1.2,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "2",
                number: 2,
                x: 67,
                y: 10,
                height: 1.2,
                type: "vertical",
                rotation: -0.7853981633974481,
                manualRotation: true
            },
            {
                id: "3A",
                number: "3A",
                x: 54.2,
                y: 36,
                height: 1.2,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "3B",
                number: "3B",
                x: 47.1,
                y: 30,
                height: 1.2,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "4",
                number: 4,
                x: 31.2,
                y: 17,
                height: 1.2,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "5",
                number: 5,
                x: 22.1,
                y: 30,
                height: 1.2,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "6",
                number: 6,
                x: 54.2,
                y: 10,
                height: 1.2,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "7",
                number: 7,
                x: 72,
                y: 32,
                height: 1.2,
                type: "vertical",
                rotation: 0.5235987755982987,
                manualRotation: true
            },
            {
                id: "8A",
                number: "8A",
                x: 45.2,
                y: 45,
                height: 1.2,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "8B",
                number: "8B",
                x: 35.1,
                y: 45,
                height: 1.2,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "8C",
                number: "8C",
                x: 26.2,
                y: 45,
                height: 1.2,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "9",
                number: 9,
                x: 39.1,
                y: 13,
                height: 1.2,
                type: "vertical",
                rotation: -0.7853981633974481,
                manualRotation: true
            },
            {
                id: "10",
                number: 10,
                x: 50.2,
                y: 19,
                height: 1.2,
                type: "vertical",
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: "11",
                number: 11,
                x: 65.1,
                y: 20,
                height: 1.2,
                type: "vertical",
                rotation: 1.5707963267948963
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3A", "3B"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8A", "8B", "8C"],
                type: "triple"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    // Advanced amateur Showjumping Courses
    {
        id: "advanced_amateur_showjumping_course",
        name: "advanced_amateur Showjumping Course",
        competition_level: "medium",
        jumping_discipline: 'showjumping',
        arena: { width: 90, length: 55 },
        jumps: [
            {
                id: "1",
                number: 1,
                x: 12,
                y: 34,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -0.2617993877991495,
                manualRotation: true
            },
            {
                id: "2",
                number: 2,
                x: 35,
                y: 41,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: "3a",
                number: "3a",
                x: 50,
                y: 32,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "3b",
                number: "3b",
                x: 56,
                y: 26,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "4",
                number: 4,
                x: 72,
                y: 48,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "5",
                number: 5,
                x: 48,
                y: 49,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "6",
                number: 6,
                x: 20,
                y: 44,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: "7",
                number: 7,
                x: 16,
                y: 22,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "8a",
                number: "8a",
                x: 41,
                y: 7,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "8b",
                number: "8b",
                x: 49,
                y: 7,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "9",
                number: 9,
                x: 75,
                y: 17,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -0.5235987755982988
            },
            {
                id: "10",
                number: 10,
                x: 70,
                y: 39,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: 2.0943951023931953
            },
            {
                id: "11",
                number: 11,
                x: 61,
                y: 35,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -0.7853981633974483
            },
            {
                id: "12",
                number: 12,
                x: 43,
                y: 22,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -0.7853981633974483
            },
            {
                id: "13",
                number: 13,
                x: 25,
                y: 11,
                height: 1.0499999999999998,
                type: "vertical",
                rotation: -1.308996938995747
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3a", "3b"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8a", "8b"],
                type: "triple"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
        }
    },
    {
        id: "medium_showjumping_var_4_mirror_v_position",
        name: "Medium Level Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "medium",
        jumping_discipline: 'showjumping',
        arena: { width: 90, length: 55 },
        jumps: [
            {
                id: "1",
                number: 1,
                x: 13.2,
                y: 21,
                height: 1.25,
                type: "vertical",
                rotation: -2.6179938779914944,
                manualRotation: true
            },
            {
                id: "2",
                number: 2,
                x: 36.1,
                y: 14,
                height: 1.25,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "3A",
                number: "3A",
                x: 51.2,
                y: 23,
                height: 1.25,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "3B",
                number: "3B",
                x: 57.1,
                y: 29,
                height: 1.25,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "4",
                number: 4,
                x: 73.2,
                y: 7,
                height: 1.25,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "5",
                number: 5,
                x: 49.1,
                y: 6,
                height: 1.25,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "6",
                number: 6,
                x: 21.2,
                y: 11,
                height: 1.25,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "7",
                number: 7,
                x: 17,
                y: 33,
                height: 1.25,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "8A",
                number: "8A",
                x: 42.2,
                y: 48,
                height: 1.25,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "8B",
                number: "8B",
                x: 50.1,
                y: 48,
                height: 1.25,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "9",
                number: 9,
                x: 76,
                y: 38,
                height: 1.25,
                type: "vertical",
                rotation: 0.5235987755982989,
                manualRotation: true
            },
            {
                id: "10",
                number: 10,
                x: 71.1,
                y: 16,
                height: 1.25,
                type: "vertical",
                rotation: 1.0471975511965979,
                manualRotation: true
            },
            {
                id: "11",
                number: 11,
                x: 62,
                y: 20,
                height: 1.25,
                type: "vertical",
                rotation: -2.356194490192345,
                manualRotation: true
            },
            {
                id: "12",
                number: 12,
                x: 44.1,
                y: 33,
                height: 1.25,
                type: "vertical",
                rotation: -2.356194490192345,
                manualRotation: true
            },
            {
                id: "13",
                number: 13,
                x: 26.2,
                y: 44,
                height: 1.25,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3A", "3B"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8A", "8B"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 13,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "advanced_amateur_showjumping_var_2_mirror_h",
        name: "advanced_amateur Level Showjumping Course - Mirrored Horizontal",
        competition_level: "medium",
        jumping_discipline: 'showjumping',
        arena: { width: 90, length: 55 },
        jumps: [
            {
                id: "1",
                number: 1,
                x: 78,
                y: 34,
                height: 1.25,
                type: "vertical",
                rotation: 0.2617993877991495,
                manualRotation: true
            },
            {
                id: "2",
                number: 2,
                x: 55,
                y: 41,
                height: 1.25,
                type: "vertical",
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            // COMBINATION 3A-3B (Mirrored together as a unit)
            {
                id: "3A",
                number: "3a",
                x: 40,
                y: 32,
                height: 1.25,
                type: "vertical",
                rotation: -0.7853981633974483,
                combination: "3"
            },
            {
                id: "3B",
                number: "3b",
                x: 34,
                y: 26,
                height: 1.25,
                type: "vertical",
                rotation: -0.7853981633974483,
                combination: "3"
            },
            {
                id: "4",
                number: 4,
                x: 18,
                y: 48,
                height: 1.25,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "5",
                number: 5,
                x: 42,
                y: 49,
                height: 1.25,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "6",
                number: 6,
                x: 70,
                y: 44,
                height: 1.25,
                type: "vertical",
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: "7",
                number: 7,
                x: 74,
                y: 22,
                height: 1.25,
                type: "vertical",
                rotation: -1.0471975511965976
            },
            // COMBINATION 8A-8B (Mirrored together as a unit)
            {
                id: "8A",
                number: "8a",
                x: 49,
                y: 7,
                height: 1.25,
                type: "vertical",
                rotation: -1.5707963267948963,
                combination: "8"
            },
            {
                id: "8B",
                number: "8b",
                x: 41,
                y: 7,
                height: 1.25,
                type: "vertical",
                rotation: -1.5707963267948963,
                combination: "8"
            },
            {
                id: "9",
                number: 9,
                x: 15,
                y: 17,
                height: 1.25,
                type: "vertical",
                rotation: 0.5235987755982988
            },
            {
                id: "10",
                number: 10,
                x: 20,
                y: 39,
                height: 1.25,
                type: "vertical",
                rotation: -2.0943951023931953
            },
            {
                id: "11",
                number: 11,
                x: 29,
                y: 35,
                height: 1.25,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "12",
                number: 12,
                x: 47,
                y: 22,
                height: 1.25,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "13",
                number: 13,
                x: 65,
                y: 11,
                height: 1.25,
                type: "vertical",
                rotation: 1.308996938995747
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3a", "3b"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8a", "8b"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 13,
            variation: "Horizontal mirror flip"
        }
    },

    // Advanced Showjumping Courses
    {
        id: "advanced_showjumping_course",
        name: "advanced Showjumping Course",
        competition_level: "advanced",
        jumping_discipline: 'showjumping',
        arena: { width: 100, length: 60 },
        jumps: [
            {
                id: 1752062035937,
                x: 43,
                y: 8,
                type: "vertical",
                number: 1,
                height: 1.2000000000000002,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752062051406,
                x: 75,
                y: 26,
                type: "vertical",
                number: 2,
                height: 1.2000000000000002,
                rotation: 0.2617993877991494,
                manualRotation: true
            },
            {
                id: 1752062056667,
                x: 95,
                y: 30,
                type: "vertical",
                number: 3,
                height: 1.2000000000000002,
                rotation: 1.1102230246251565e-16,
                manualRotation: true
            },
            {
                id: 1752062062128,
                x: 62,
                y: 18,
                type: "vertical",
                number: 4,
                height: 1.2000000000000002,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062085239,
                x: 44,
                y: 39,
                type: "vertical",
                number: '5a',
                height: 1.2000000000000002,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062087357,
                x: 38,
                y: 45,
                type: "vertical",
                number: '5b',
                height: 1.2000000000000002,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062101022,
                x: 20,
                y: 45,
                type: "vertical",
                number: 6,
                height: 1.2000000000000002,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062124244,
                x: 52,
                y: 55,
                type: "vertical",
                number: 7,
                height: 1.2000000000000002,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752062132440,
                x: 80,
                y: 51,
                type: "vertical",
                number: 8,
                height: 1.2000000000000002,
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1752062150184,
                x: 60,
                y: 39,
                type: "vertical",
                number: '9a',
                height: 1.2000000000000002,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062152439,
                x: 55,
                y: 45,
                type: "vertical",
                number: '9b',
                height: 1.2000000000000002,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062163722,
                x: 11,
                y: 41,
                type: "vertical",
                number: 10,
                height: 1.2000000000000002,
                rotation: 0
            },
            {
                id: 1752062167704,
                x: 17,
                y: 16,
                type: "vertical",
                number: 11,
                height: 1.2000000000000002,
                rotation: 0.2617993877991495,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "3",
                jumps: ["3a", "3b"],
                type: "double"
            },
            {
                id: "8",
                jumps: ["8a", "8b"],
                type: "triple"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "advanced_showjumping_var_4_mirror_v_position",
        name: "Advanced Showjumping Course - Vertical Mirror + Position Tweaks",
        competition_level: "advanced",
        jumping_discipline: 'showjumping',
        arena: { width: 100, length: 60 },
        jumps: [
            {
                id: "1",
                number: 1,
                x: 44.2,
                y: 52,
                height: 1.4,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "2",
                number: 2,
                x: 76.1,
                y: 34,
                height: 1.4,
                type: "vertical",
                rotation: -0.2617993877991494,
                manualRotation: true
            },
            {
                id: "3",
                number: 3,
                x: 96.2,
                y: 30,
                height: 1.4,
                type: "vertical",
                rotation: 0
            },
            {
                id: "4",
                number: 4,
                x: 63.1,
                y: 42,
                height: 1.4,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "5A",
                number: "5A",
                x: 45.2,
                y: 21,
                height: 1.4,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "5B",
                number: "5B",
                x: 39.1,
                y: 15,
                height: 1.4,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "6",
                number: 6,
                x: 21.2,
                y: 15,
                height: 1.4,
                type: "vertical",
                rotation: 0.5235987755982987,
                manualRotation: true
            },
            {
                id: "7",
                number: 7,
                x: 53.1,
                y: 5,
                height: 1.4,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "8",
                number: 8,
                x: 81,
                y: 9,
                height: 1.4,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "9A",
                number: "9A",
                x: 61.1,
                y: 21,
                height: 1.4,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "9B",
                number: "9B",
                x: 55,
                y: 15,
                height: 1.4,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "10",
                number: 10,
                x: 12.1,
                y: 19,
                height: 1.4,
                type: "vertical",
                rotation: 0
            },
            {
                id: "11",
                number: 11,
                x: 18.2,
                y: 44,
                height: 1.4,
                type: "vertical",
                rotation: -0.5235987755982987,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "5",
                jumps: ["5A", "5B"],
                type: "double"
            },
            {
                id: "9",
                jumps: ["9A", "9B"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "advanced_showjumping_var_2_mirror_h",
        name: "Advanced Showjumping Course - Mirrored Horizontal",
        competition_level: "advanced",
        jumping_discipline: 'showjumping',
        arena: { width: 100, length: 60 },
        jumps: [
            {
                id: 1752062035939,
                x: 57,
                y: 8,
                type: "vertical",
                number: 1,
                height: 1.40,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752062051408,
                x: 25,
                y: 26,
                type: "vertical",
                number: 2,
                height: 1.40,
                rotation: -0.2617993877991494,
                manualRotation: true
            },
            {
                id: 1752062056669,
                x: 5,
                y: 30,
                type: "vertical",
                number: 3,
                height: 1.40,
                rotation: 0,
                manualRotation: true
            },
            {
                id: 1752062062130,
                x: 38,
                y: 18,
                type: "vertical",
                number: 4,
                height: 1.40,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            // COMBINATION 5A-5B (Mirrored together as a unit)
            {
                id: 1752062085241,
                x: 56,
                y: 39,
                type: "vertical",
                number: '5A',
                height: 1.40,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "5"
            },
            {
                id: 1752062087359,
                x: 62,
                y: 45,
                type: "vertical",
                number: '5B',
                height: 1.40,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "5"
            },
            {
                id: 1752062101024,
                x: 80,
                y: 45,
                type: "vertical",
                number: 6,
                height: 1.40,
                rotation: 0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1752062124246,
                x: 48,
                y: 55,
                type: "vertical",
                number: 7,
                height: 1.40,
                rotation: 1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1752062132442,
                x: 20,
                y: 51,
                type: "vertical",
                number: 8,
                height: 1.40,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            // COMBINATION 9A-9B (Mirrored together as a unit)
            {
                id: 1752062150186,
                x: 40,
                y: 39,
                type: "vertical",
                number: '9A',
                height: 1.40,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "9"
            },
            {
                id: 1752062152441,
                x: 45,
                y: 45,
                type: "vertical",
                number: '9B',
                height: 1.40,
                rotation: -0.7853981633974483,
                manualRotation: true,
                combination: "9"
            },
            {
                id: 1752062163724,
                x: 89,
                y: 41,
                type: "vertical",
                number: 10,
                height: 1.40,
                rotation: 0
            },
            {
                id: 1752062167706,
                x: 83,
                y: 16,
                type: "vertical",
                number: 11,
                height: 1.40,
                rotation: -0.2617993877991495,
                manualRotation: true
            }
        ],
        combinations: [
            {
                id: "5",
                jumps: ["5A", "5B"],
                type: "double"
            },
            {
                id: "9",
                jumps: ["9A", "9B"],
                type: "double"
            }
        ],
        metadata: {
            aiGenerated: false,
            designMode: "variation",
            createdAt: "2025-07-09T08:44:06.597Z",
            totalJumps: 11,
            variation: "Horizontal mirror flip"
        }
    },

    // Grand Prix Showjumping Courses
    {
        id: "macon_grand_indoor_135",
        name: "Macon Grand Indoor 1.35m",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: 1751532792875,
                x: 41,
                y: 17,
                type: "vertical",
                number: 1,
                height: 0.8,
                rotation: -2.094395102393195,
                manualRotation: true
            },
            {
                id: 1751532809192,
                x: 12,
                y: 27,
                type: "vertical",
                number: 2,
                height: 0.8,
                rotation: -1.0471975511965976,
                manualRotation: true
            },
            {
                id: 1751532810147,
                x: 20,
                y: 20,
                type: "vertical",
                number: 3,
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1751532811125,
                x: 48,
                y: 27,
                type: "vertical",
                number: 4,
                height: 0.8,
                rotation: -1.832595714594046,
                manualRotation: true
            },
            {
                id: 1751532846324,
                x: 48,
                y: 5,
                type: "vertical",
                number: 5,
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1751532976098,
                x: 25,
                y: 5,
                type: "vertical",
                number: '6a',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1751532982270,
                x: 15,
                y: 5,
                type: "vertical",
                number: '6b',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1751533030213,
                x: 23,
                y: 14,
                type: "vertical",
                number: 7,
                height: 0.8,
                rotation: 1.308996938995747,
                manualRotation: true
            },
            {
                id: 1751533060406,
                x: 49,
                y: 14,
                type: "vertical",
                number: 8,
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1751533080047,
                x: 37,
                y: 23,
                type: "vertical",
                number: '9a',
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1751533082760,
                x: 31,
                y: 16,
                type: "vertical",
                number: '9b',
                height: 0.8,
                rotation: -0.7853981633974483,
                manualRotation: true
            },
            {
                id: 1751533094872,
                x: 12,
                y: 12,
                type: "vertical",
                number: 10,
                height: 0.8,
                rotation: 1.0471975511965972,
                manualRotation: true
            },
            {
                id: 1751533119062,
                x: 17,
                y: 34,
                type: "vertical",
                number: '11a',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1751533127865,
                x: 27,
                y: 34,
                type: "vertical",
                number: '11b',
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            },
            {
                id: 1751533132936,
                x: 45,
                y: 34,
                type: "vertical",
                number: 12,
                height: 0.8,
                rotation: -1.5707963267948963,
                manualRotation: true
            }
        ],
        metadata: {
            source: "Macon Grand Indoor de France 2019",
            complexity: "high",
            pattern: "serpentine",
            designer: "Cédric LONGIS, Stephane Paillot"
        }
    },
    {
        id: "macon_grand_indoor_var_1_rotate_15",
        name: "Macon Grand Indoor 1.35m - Rotated 15°",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 36,
                y: 21,
                height: 1.35,
                type: "vertical",
                rotation: -1.8326
            },
            {
                id: "jump_2",
                number: 2,
                x: 7.1,
                y: 24.9,
                height: 1.35,
                type: "vertical",
                rotation: -0.7854
            },
            {
                id: "jump_3",
                number: 3,
                x: 16,
                y: 17,
                height: 1.35,
                type: "vertical",
                rotation: -0.5236
            },
            {
                id: "jump_4",
                number: 4,
                x: 41,
                y: 28,
                height: 1.35,
                type: "vertical",
                rotation: -2.094398775598299,
                manualRotation: true
            },
            {
                id: "jump_5",
                number: 5,
                x: 49.3,
                y: 9.7,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_6a",
                number: "6a",
                x: 26,
                y: 8,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_6b",
                number: "6b",
                x: 17,
                y: 6,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_7",
                number: 7,
                x: 21,
                y: 19,
                height: 1.35,
                type: "vertical",
                rotation: 0.7854018366025519,
                manualRotation: true
            },
            {
                id: "jump_8",
                number: 8,
                x: 46.3,
                y: 18.6,
                height: 1.35,
                type: "vertical",
                rotation: -0.5236
            },
            {
                id: "jump_9a",
                number: "9a",
                x: 33,
                y: 26,
                height: 1.35,
                type: "vertical",
                rotation: -0.7853993877991494,
                manualRotation: true
            },
            {
                id: "jump_9b",
                number: "9b",
                x: 27.3,
                y: 19.1,
                height: 1.35,
                type: "vertical",
                rotation: -0.5236
            },
            {
                id: "jump_10",
                number: 10,
                x: 8.9,
                y: 9.8,
                height: 1.35,
                type: "vertical",
                rotation: 1.309
            },
            {
                id: "jump_11",
                number: '11a',
                x: 12.3,
                y: 32.1,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_12",
                number: '11b',
                x: 22,
                y: 32.9,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_13",
                number: 12,
                x: 35,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: -1.309
            }
        ],
        metadata: {
            source: "Macon Grand Indoor de France 2019 - Variation",
            complexity: "high",
            pattern: "serpentine rotated",
            designer: "Cédric LONGIS, Stephane Paillot",
            variation: "15° clockwise rotation"
        }
    },
    {
        id: "macon_grand_indoor_var_2_mirror_h",
        name: "Macon Grand Indoor 1.35m - Mirrored Horizontal",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 19,
                y: 17,
                height: 1.35,
                type: "vertical",
                rotation: 2.094395102393195
            },
            {
                id: "jump_2",
                number: 2,
                x: 48,
                y: 27,
                height: 1.35,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "jump_3",
                number: 3,
                x: 40,
                y: 20,
                height: 1.35,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "jump_4",
                number: 4,
                x: 12,
                y: 27,
                height: 1.35,
                type: "vertical",
                rotation: 1.832595714594046
            },
            {
                id: "jump_5",
                number: 5,
                x: 12,
                y: 5,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_6a",
                number: "6a",
                x: 35,
                y: 5,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_6b",
                number: "6b",
                x: 45,
                y: 5,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_7",
                number: 7,
                x: 37,
                y: 14,
                height: 1.35,
                type: "vertical",
                rotation: -1.308996938995747
            },
            {
                id: "jump_8",
                number: 8,
                x: 11,
                y: 14,
                height: 1.35,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "jump_9a",
                number: "9a",
                x: 23,
                y: 23,
                height: 1.35,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "jump_9b",
                number: "9b",
                x: 29,
                y: 16,
                height: 1.35,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "jump_10",
                number: 10,
                x: 50,
                y: 14,
                height: 1.35,
                type: "vertical",
                rotation: -0.7853981633974478,
                manualRotation: true
            },
            {
                id: "jump_11",
                number: '11a',
                x: 43,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_12",
                number: '11b',
                x: 33,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_13",
                number: 12,
                x: 16,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: 1.5707963267948963
            }
        ],
        metadata: {
            source: "Macon Grand Indoor de France 2019 - Variation",
            complexity: "high",
            pattern: "serpentine mirrored",
            designer: "Cédric LONGIS, Stephane Paillot",
            variation: "Horizontal mirror flip"
        }
    },
    {
        id: "macon_grand_indoor_var_3_mirror_v_position",
        name: "Macon Grand Indoor 1.35m - Vertical Mirror + Position Tweaks",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 40,
                y: 25,
                height: 1.35,
                type: "vertical",
                rotation: -3.926990816987242,
                manualRotation: true
            },
            {
                id: "jump_2",
                number: 2,
                x: 12,
                y: 14,
                height: 1.35,
                type: "vertical",
                rotation: -1.832595714594046,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 21.2,
                y: 20,
                height: 1.35,
                type: "vertical",
                rotation: 0.5235987755982987,
                manualRotation: true
            },
            {
                id: "jump_4",
                number: 4,
                x: 49,
                y: 13,
                height: 1.35,
                type: "vertical",
                rotation: -1.3089969389957472,
                manualRotation: true
            },
            {
                id: "jump_5",
                number: 5,
                x: 49,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_6a",
                number: "6a",
                x: 23.8,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_6b",
                number: "6b",
                x: 16.2,
                y: 35,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_7",
                number: 7,
                x: 22,
                y: 25,
                height: 1.35,
                type: "vertical",
                rotation: 2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_8",
                number: 8,
                x: 49,
                y: 27,
                height: 1.35,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_9a",
                number: "9a",
                x: 38.7,
                y: 17,
                height: 1.35,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_9b",
                number: "9b",
                x: 31,
                y: 24,
                height: 1.35,
                type: "vertical",
                rotation: -2.356194490192345,
                manualRotation: true
            },
            {
                id: "jump_10",
                number: 10,
                x: 12,
                y: 29,
                height: 1.35,
                type: "vertical",
                rotation: -1.3089969389957472,
                manualRotation: true
            },
            {
                id: "jump_11",
                number: '11a',
                x: 18.3,
                y: 6,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_12",
                number: '11b',
                x: 26.7,
                y: 6,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_13",
                number: 12,
                x: 44.2,
                y: 6,
                height: 1.35,
                type: "vertical",
                rotation: -1.5707963267948963
            }
        ],
        metadata: {
            source: "Macon Grand Indoor de France 2019 - Variation",
            complexity: "high",
            pattern: "serpentine vertically mirrored with position adjustments",
            designer: "Cédric LONGIS, Stephane Paillot",
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "macon_grand_indoor_var_4_rotate_30_height",
        name: "Macon Grand Indoor 1.35m - Rotated 30° + Height Changes",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 41,
                y: 16,
                height: 1.3,
                type: "vertical",
                rotation: -2.617997551196598,
                manualRotation: true
            },
            {
                id: "jump_2",
                number: 2,
                x: 7,
                y: 25,
                height: 1.4,
                type: "vertical",
                rotation: -1.0471987755982988,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 14,
                y: 15,
                height: 1.35,
                type: "vertical",
                rotation: -0.2618
            },
            {
                id: "jump_4",
                number: 4,
                x: 44,
                y: 31,
                height: 1.25,
                type: "vertical",
                rotation: -1.309
            },
            {
                id: "jump_5",
                number: 5,
                x: 49.6,
                y: 12.3,
                height: 1.45,
                type: "vertical",
                rotation: -1.0472
            },
            {
                id: "jump_6a",
                number: "6a",
                x: 35,
                y: 9,
                height: 1.4,
                type: "vertical",
                rotation: -1.0472,
                manualRotation: true
            },
            {
                id: "jump_6b",
                number: "6b",
                x: 25,
                y: 7,
                height: 1.4,
                type: "vertical",
                rotation: -1.5707987755982986,
                manualRotation: true
            },
            {
                id: "jump_7",
                number: 7,
                x: 18,
                y: 19,
                height: 1.3,
                type: "vertical",
                rotation: 1.047201836602552,
                manualRotation: true
            },
            {
                id: "jump_8",
                number: 8,
                x: 50,
                y: 21,
                height: 1.35,
                type: "vertical",
                rotation: -0.2618
            },
            {
                id: "jump_9a",
                number: "9a",
                x: 32,
                y: 27,
                height: 1.25,
                type: "vertical",
                rotation: -0.7853987755982987,
                manualRotation: true
            },
            {
                id: "jump_9b",
                number: "9b",
                x: 26,
                y: 21,
                height: 1.25,
                type: "vertical",
                rotation: -0.7853987755982987,
                manualRotation: true
            },
            {
                id: "jump_10",
                number: 10,
                x: 8.4,
                y: 8,
                height: 1.4,
                type: "vertical",
                rotation: 1.5708
            },
            {
                id: "jump_11",
                number: '11a',
                x: 7.6,
                y: 32.6,
                height: 1.35,
                type: "vertical",
                rotation: -1.0472
            },
            {
                id: "jump_12",
                number: '11b',
                x: 16.2,
                y: 35.4,
                height: 1.3,
                type: "vertical",
                rotation: -1.5707987755982986,
                manualRotation: true
            },
            {
                id: "jump_13",
                number: 12,
                x: 33,
                y: 35,
                height: 1.45,
                type: "vertical",
                rotation: -1.5707987755982986,
                manualRotation: true
            }
        ],
        metadata: {
            source: "Macon Grand Indoor de France 2019 - Variation",
            complexity: "high",
            pattern: "serpentine rotated 30° with height variation",
            designer: "Cédric LONGIS, Stephane Paillot",
            variation: "30° clockwise rotation + height range 1.25m-1.45m"
        }
    },
    {
        id: "jersey_fresh_cci3",
        name: "Jersey Fresh CCI 3* Cross Country",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 }, // Cross country - larger area
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 45,
                y: 10,
                height: 0.8,
                type: "vertical",
                rotation: -0.7853981633974483
            },
            {
                id: "jump_2",
                number: 2,
                x: 43,
                y: 25,
                height: 0.8,
                type: "vertical",
                rotation: 2.356194490192345
            },
            {
                id: "jump_3",
                number: 3,
                x: 34,
                y: 15,
                height: 0.8,
                type: "vertical",
                rotation: -0.7853981633974481
            },
            {
                id: "jump_4",
                number: 4,
                x: 14,
                y: 7,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_5",
                number: 5,
                x: 12,
                y: 33,
                height: 0.8,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "jump_6",
                number: 6,
                x: 29,
                y: 22,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_7",
                number: 7,
                x: 44,
                y: 35,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_8",
                number: 8,
                x: 30,
                y: 35,
                height: 0.8,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_9",
                number: 9,
                x: 21,
                y: 15,
                height: 0.8,
                type: "vertical",
                rotation: 0.5235987755982988
            },
            {
                id: "jump_10",
                number: 10,
                x: 11,
                y: 21,
                height: 0.8,
                type: "vertical",
                rotation: -1.0471975511965976
            },
            {
                id: "jump_11",
                number: 11,
                x: 24,
                y: 29,
                height: 0.8,
                type: "vertical",
                rotation: -1.0471975511965976
            },
            {
                id: "jump_12",
                number: 12,
                x: 30,
                y: 30,
                height: 0.8,
                type: "vertical",
                rotation: -1.0471975511965976
            },
            {
                id: "jump_13",
                number: 13,
                x: 37,
                y: 30,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_14",
                number: 14,
                x: 53,
                y: 24,
                height: 0.8,
                type: "vertical",
                rotation: 1.0471975511965976
            }
        ],
        metadata: {
            source: "Jersey Fresh International Three Day Event",
            complexity: "high",
            pattern: "technical",
            designer: "International Course Designer"
        }
    },
    {
        id: "jersey_fresh_cci3_var_1_rotate_25",
        name: "Jersey Fresh CCI 3* Cross Country - Rotated 25°",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 44,
                y: 13,
                height: 1.15,
                type: "vertical",
                rotation: -1.1344981633974482,
                manualRotation: true
            },
            {
                id: "jump_2",
                number: 2,
                x: 41,
                y: 28,
                height: 1.15,
                type: "vertical",
                rotation: 2.268901224401701,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 29,
                y: 15,
                height: 1.15,
                type: "vertical",
                rotation: -0.8726987755982989,
                manualRotation: true
            },
            {
                id: "jump_4",
                number: 4,
                x: 19,
                y: 9,
                height: 1.15,
                type: "vertical",
                rotation: -1.1345
            },
            {
                id: "jump_5",
                number: 5,
                x: 10,
                y: 31,
                height: 1.15,
                type: "vertical",
                rotation: 1.4835
            },
            {
                id: "jump_6",
                number: 6,
                x: 23,
                y: 26,
                height: 1.15,
                type: "vertical",
                rotation: -2.1816975511965975,
                manualRotation: true
            },
            {
                id: "jump_7",
                number: '7a',
                x: 34,
                y: 35,
                height: 1.15,
                type: "vertical",
                rotation: -1.6580987755982988,
                manualRotation: true
            },
            {
                id: "jump_8",
                number: '7b',
                x: 27,
                y: 35,
                height: 1.15,
                type: "vertical",
                rotation: 1.4835012244017012,
                manualRotation: true
            },
            {
                id: "jump_9",
                number: 8,
                x: 18,
                y: 20,
                height: 1.15,
                type: "vertical",
                rotation: 0.4363012244017012,
                manualRotation: true
            },
            {
                id: "jump_10",
                number: 9,
                x: 9,
                y: 19,
                height: 1.15,
                type: "vertical",
                rotation: -0.6109
            },
            {
                id: "jump_11",
                number: '10a',
                x: 15,
                y: 25,
                height: 1.15,
                type: "vertical",
                rotation: -0.6109
            },
            {
                id: "jump_12",
                number: '10b',
                x: 24,
                y: 31,
                height: 1.15,
                type: "vertical",
                rotation: -1.9198969389957468,
                manualRotation: true
            },
            {
                id: "jump_13",
                number: '10c',
                x: 34,
                y: 26,
                height: 1.15,
                type: "vertical",
                rotation: -2.1816975511965975,
                manualRotation: true
            },
            {
                id: "jump_14",
                number: 11,
                x: 47,
                y: 20,
                height: 1.15,
                type: "vertical",
                rotation: 0.698101836602552,
                manualRotation: true
            }
        ],
        metadata: {
            source: "Jersey Fresh International Three Day Event - Variation",
            complexity: "high",
            pattern: "technical rotated",
            designer: "International Course Designer",
            variation: "25° clockwise rotation"
        }
    },
    {
        id: "jersey_fresh_cci3_var_2_mirror_h",
        name: "Jersey Fresh CCI 3* Cross Country - Mirrored Horizontal",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 15,
                y: 10,
                height: 1.15,
                type: "vertical",
                rotation: 0.7853981633974483
            },
            {
                id: "jump_2",
                number: 2,
                x: 17,
                y: 25,
                height: 1.15,
                type: "vertical",
                rotation: -2.356194490192345
            },
            {
                id: "jump_3",
                number: 3,
                x: 26,
                y: 15,
                height: 1.15,
                type: "vertical",
                rotation: 0.7853981633974481
            },
            {
                id: "jump_4",
                number: 4,
                x: 46,
                y: 7,
                height: 1.15,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_5",
                number: 5,
                x: 48,
                y: 33,
                height: 1.15,
                type: "vertical",
                rotation: -1.0471975511965976
            },
            {
                id: "jump_6",
                number: 6,
                x: 31,
                y: 22,
                height: 1.15,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_7",
                number: '7a',
                x: 16,
                y: 35,
                height: 1.15,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_8",
                number: '7b',
                x: 30,
                y: 35,
                height: 1.15,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_9",
                number: 8,
                x: 39,
                y: 15,
                height: 1.15,
                type: "vertical",
                rotation: -0.5235987755982988
            },
            {
                id: "jump_10",
                number: 9,
                x: 49,
                y: 21,
                height: 1.15,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "jump_11",
                number: '10a',
                x: 36,
                y: 29,
                height: 1.15,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "jump_12",
                number: '10b',
                x: 30,
                y: 30,
                height: 1.15,
                type: "vertical",
                rotation: 1.0471975511965976
            },
            {
                id: "jump_13",
                number: '10c',
                x: 23,
                y: 30,
                height: 1.15,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_14",
                number: 11,
                x: 7,
                y: 24,
                height: 1.15,
                type: "vertical",
                rotation: -1.0471975511965976
            }
        ],
        metadata: {
            source: "Jersey Fresh International Three Day Event - Variation",
            complexity: "high",
            pattern: "technical mirrored",
            designer: "International Course Designer",
            variation: "Horizontal mirror flip"
        }
    },
    {
        id: "jersey_fresh_cci3_var_3_mirror_v_position",
        name: "Jersey Fresh CCI 3* Cross Country - Vertical Mirror + Position Tweaks",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 60, length: 40 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 46.2,
                y: 30,
                height: 1.15,
                type: "vertical",
                rotation: -2.356194490192345,
                manualRotation: true
            },
            {
                id: "jump_2",
                number: 2,
                x: 44.1,
                y: 15,
                height: 1.15,
                type: "vertical",
                rotation: 0.7853981633974485,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 35.3,
                y: 25,
                height: 1.15,
                type: "vertical",
                rotation: -2.3561944901923444,
                manualRotation: true
            },
            {
                id: "jump_4",
                number: 4,
                x: 15.1,
                y: 33,
                height: 1.15,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_5",
                number: 5,
                x: 14,
                y: 10,
                height: 1.15,
                type: "vertical",
                rotation: 2.356194490192345,
                manualRotation: true
            },
            {
                id: "jump_6",
                number: 6,
                x: 30.8,
                y: 18,
                height: 1.15,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_7",
                number: 7,
                x: 45.2,
                y: 5,
                height: 1.15,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_8",
                number: 8,
                x: 31.3,
                y: 5,
                height: 1.15,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_9",
                number: 9,
                x: 20,
                y: 24,
                height: 1.15,
                type: "vertical",
                rotation: -0.2617993877991494,
                manualRotation: true
            },
            {
                id: "jump_10",
                number: 10,
                x: 13,
                y: 20,
                height: 1.15,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_11",
                number: 11,
                x: 26,
                y: 14,
                height: 1.15,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_12",
                number: 12,
                x: 33,
                y: 11,
                height: 1.15,
                type: "vertical",
                rotation: -2.0943951023931953,
                manualRotation: true
            },
            {
                id: "jump_13",
                number: 13,
                x: 40,
                y: 10,
                height: 1.15,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_14",
                number: 14,
                x: 52,
                y: 19,
                height: 1.15,
                type: "vertical",
                rotation: 2.6179938779914944,
                manualRotation: true
            }
        ],
        metadata: {
            source: "Jersey Fresh International Three Day Event - Variation",
            complexity: "high",
            pattern: "technical vertically mirrored with position adjustments",
            designer: "International Course Designer",
            variation: "Vertical mirror + 1-2m position adjustments"
        }
    },
    {
        id: "paris_grand_indoor_160",
        name: "Paris Grand Indoor 1.60m",
        competition_level: "grand_prix",
        jumping_discipline: 'showjumping',
        arena: { width: 100, length: 60 },
        jumps: [
            {
                id: "jump_1",
                number: 1,
                x: 45,
                y: 25,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_2",
                number: 2,
                x: 39,
                y: 12,
                height: 0.8,
                type: "vertical",
                rotation: 1.0471975511965976,
                manualRotation: true
            },
            {
                id: "jump_3",
                number: 3,
                x: 17,
                y: 20,
                height: 0.8,
                type: "vertical",
                rotation: 1.308996938995747
            },
            {
                id: "jump_4a",
                number: "4a",
                x: 14,
                y: 5,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_4b",
                number: "4b",
                x: 22,
                y: 5,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_4c",
                number: "4c",
                x: 34,
                y: 5,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_5",
                number: 5,
                x: 47,
                y: 5,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_6",
                number: 6,
                x: 45,
                y: 31,
                height: 0.8,
                type: "vertical",
                rotation: 1.8325957145940457
            },
            {
                id: "jump_7",
                number: 7,
                x: 29,
                y: 21,
                height: 0.8,
                type: "vertical",
                rotation: 2.0943951023931953
            },
            {
                id: "jump_8",
                number: 8,
                x: 12,
                y: 14,
                height: 0.8,
                type: "vertical",
                rotation: 1.308996938995747
            },
            {
                id: "jump_9",
                number: 9,
                x: 23,
                y: 30,
                height: 0.8,
                type: "vertical",
                rotation: 1.308996938995747
            },
            {
                id: "jump_10a",
                number: "10a",
                x: 37,
                y: 22,
                height: 0.8,
                type: "vertical",
                rotation: 1.308996938995747,
                manualRotation: true
            },
            {
                id: "jump_10b",
                number: "10b",
                x: 44,
                y: 16,
                height: 0.8,
                type: "vertical",
                rotation: 1.308996938995747,
                manualRotation: true
            },
            {
                id: "jump_11a",
                number: "11a",
                x: 41,
                y: 35,
                height: 0.8,
                type: "vertical",
                rotation: 1.5707963267948963
            },
            {
                id: "jump_11b",
                number: "11b",
                x: 30,
                y: 35,
                height: 0.8,
                type: "vertical",
                rotation: -1.5707963267948963
            },
            {
                id: "jump_12",
                number: 12,
                x: 8,
                y: 26,
                height: 0.8,
                type: "vertical",
                rotation: -0.2617993877991494
            }
        ],
        metadata: {
            source: "Paris Grand Indoor Salon du Cheval 2018",
            complexity: "high",
            pattern: "flowing",
            designer: "Cédric LONGIS, Stéphane Paillot, Gauthier Delafontaine"
        }
    }
]