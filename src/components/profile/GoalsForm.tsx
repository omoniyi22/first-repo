...
const GoalsForm = ({ onComplete, initialGoals }: GoalsFormProps) => {
  ...
  const handleGoalTextChange = (term: keyof GoalsData, id: number, text: string) => {
    setGoals({
      ...goals,
  ...
  const handleProgressChange = (term: keyof GoalsData, id: number, progress: string) => {
  ...
  const handleDateChange = (term: keyof GoalsData, id: number, date: Date | undefined) => {
    if (!date) return;
    
    setGoals({
      ...goals,
      [term]: goals[term].map(goal => 
        goal.id === id ? { ...goal, targetDate: date.toISOString().split('T')[0] } : goal
      ),
    });
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {goals.shortTerm.map((goal, index) => (
            <div key={goal.id} className="grid grid-cols-12 gap-3 items-start">
              <div className="col-span-6">
                <Label htmlFor={`short-goal-${index}`} className="text-xs mb-1 block">Goal</Label>
                <Input
                  id={`short-goal-${index}`}
                  value={goal.text}
                  onChange={(e) => handleGoalTextChange('shortTerm', goal.id, e.target.value)}
                  placeholder="Enter goal"
                />
              </div>
              <div className="col-span-2">
                <Label htmlFor={`short-progress-${index}`} className="text-xs mb-1 block">Progress %</Label>
                <Input
                  ...
                  max="100"
                  value={goal.progress}
                  onChange={(e) => handleProgressChange('shortTerm', goal.id, e.target.value)}
                />
              </div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {goal.targetDate ? format(new Date(goal.targetDate), "MM/dd/yyyy") : <span>Pick a date</span>}
                    </Button>
                  </PopoverTrigger>
        ...
        <TabsContent value="medium" className="space-y-4">
          {goals.mediumTerm.map((goal, index) => (
            <div key={goal.id} className="grid grid-cols-12 gap-3 items-start">
              <div className="col-span-6">
                <Label htmlFor={`medium-goal-${index}`} className="text-xs mb-1 block">Goal</Label>
                <Input
                  id={`medium-goal-${index}`}
                  value={goal.text}
                  onChange={(e) => handleGoalTextChange('mediumTerm', goal.id, e.target.value)}
                  placeholder="Enter goal"
                />
              </div>
              <div className="col-span-2">
                <Label htmlFor={`medium-progress-${index}`} className="text-xs mb-1 block">Progress %</Label>
                <Input
                  ...
                  max="100"
                  value={goal.progress}
                  onChange={(e) => handleProgressChange('mediumTerm', goal.id, e.target.value)}
                />
              </div>
              <div className="col-span-3">
                <Label className="text-xs mb-1 block">Target Date</Label>
        ...
        <TabsContent value="long" className="space-y-4">
          {goals.longTerm.map((goal, index) => (
            <div key={goal.id} className="grid grid-cols-12 gap-3 items-start">
              <div className="col-span-6">
                <Label htmlFor={`long-goal-${index}`} className="text-xs mb-1 block">Goal</Label>
                <Input
                  id={`long-goal-${index}`}
                  value={goal.text}
                  onChange={(e) => handleGoalTextChange('longTerm', goal.id, e.target.value)}
                  placeholder="Enter goal"
                />
              </div>
              <div className="col-span-2">
                <Label htmlFor={`long-progress-${index}`} className="text-xs mb-1 block">Progress %</Label>
                <Input
                  ...
                  max="100"
                  value={goal.progress}
                  onChange={(e) => handleProgressChange('longTerm', goal.id, e.target.value)}
                />
              </div>
              <div className="col-span-3">
                <Label className="text-xs mb-1 block">Target Date</Label>
