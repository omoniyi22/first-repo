  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button size="sm" variant="outline" onClick={() => setShowEditFocusForm(true)}>
          Edit
        </Button>
      </div>
      ...
      <Card className="border border-gray-100 p-4">
        <div className="space-y-4">
          {trainingFocus.map((focus) => (
            <div key={focus.id} className="flex items-start pb-3 border-b border-gray-100 last:border-none last:pb-0">
              <div className="bg-blue-100 text-blue-700 p-1.5 rounded mr-3">
                <Activity size={18} />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <h3 className="font-medium text-gray-900">{focus.movement}</h3>
      ...
      <Dialog open={showEditFocusForm} onOpenChange={setShowEditFocusForm}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-serif">Edit Training Focus</DialogTitle>
          </DialogHeader>
          <TrainingFocusForm onComplete={() => setShowEditFocusForm(false)} initialFocus={trainingFocus} />
        </DialogContent>
      </Dialog>
    </div>
  );
